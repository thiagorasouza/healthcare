...
function genYears(range = 50, type: "past" | "future" | "all") {
  ...
  const span = type === "all" ? range * 2 + 1 : range + 1;
  const start = type === "future" ? thisYear : thisYear - range;
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="inline-flex gap-2">
              ...
              >
                <SelectTrigger className="w-fit gap-1 border-none px-0 focus:bg-accent focus:text-accent-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {MONTHS.map((month) => (
